...
| 目标 | 成功标准 |
|------|---------|
| 为任意项目建立文档体系 | 通过 INIT 模式，能在对话引导下产出完整的 `.vision/` 文档结构 + 自动化集成配置 |
| 支撑日常开发 | BRIEF 模式能按任务需求组装完整知识链，即时补充盲区 |
| 文档与代码保持一致 | AUDIT 模式能检测变更影响并提出同步建议，支持 pre-commit 自动审计 |
| 文档质量可度量 | AUDIT 模式按多维度（含项目特定维度）输出可操作的评审报告 |
...
- **三种运行模式**：INIT（建立文档体系）→ BRIEF（获取项目知识）→ AUDIT（评估文档质量）
